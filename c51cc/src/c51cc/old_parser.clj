...
(declare ast-node-types ;; Объявление типов узлов AST
        create-parser ;; Создание парсера
        internal-parse-function-body ;; Внутренний парсер для тела функции
        internal-parse-parameters ;; Внутренний парсер для параметров функции
        parse ;; Основная функция парсинга
        parse-array-declaration ;; Парсинг объявления массива
        parse-array-initializer ;; Парсинг инициализатора массива
        parse-assignment ;; Парсинг присваивания
        parse-binary-operation ;; Парсинг бинарной операции
        parse-bitwise-operation ;; Парсинг побитовой операции
        parse-break-continue ;; Парсинг break и continue
        parse-do-while ;; Парсинг do-while цикла
        parse-expression ;; Парсинг выражений
        parse-expression-with-precedence ;; Парсинг выражений с приоритетом
        parse-for-loop ;; Парсинг for цикла
        parse-function-call ;; Парсинг вызова функции
        parse-function-declaration ;; Парсинг объявления функции
        parse-if-else ;; Парсинг if-else конструкции
        parse-pointer-declaration ;; Парсинг объявления указателя
        parse-program ;; Парсинг программы
        parse-return ;; Парсинг return
        parse-statement ;; Парсинг оператора
        parse-struct-declaration ;; Парсинг объявления структуры
        parse-switch-case ;; Парсинг switch-case конструкции
        parse-typedef ;; Парсинг typedef
        parse-unary-operation ;; Парсинг унарной операции
        parse-variable-declaration ;; Парсинг объявления переменной
        parse-while-loop ;; Парсинг while цикла
        parse-with-context ;; Парсинг с контекстом
        parse-data-space ;; Парсинг пространства данных
        parse-xdata-space ;; Парсинг пространства XDATA
        parse-code-space ;; Парсинг пространства CODE
        parse-default ;; Парсинг по умолчанию
        recognize-memory-types ;; Распознавание типов памяти
        recognize-sfr-keywords ;; Распознавание ключевых слов SFR
        recognize-bit-addressing ;; Распознавание битовой адресации
        validate-memory-constraints ;; Валидация ограничений памяти
        validate-interrupt-vectors ;; Валидация векторов прерываний
        validate-register-usage ;; Валидация использования регистров
        in-sfr-context? ;; Проверка, находимся ли в контексте SFR
        parse-bit-address ;; Парсинг битовой адресации
        parse-const-declaration ;; Парсинг объявления константы
        parse-memory-directive ;; Парсинг директивы памяти
        parse-memory-space ;; Парсинг пространства памяти
        parse-sfr-declaration ;; Парсинг объявления SFR
        parse-sfr-keyword ;; Парсинг ключевого слова SFR
        parse-type-keyword ;; Парсинг ключевого слова типа
        parse-variable-declaration ;; Парсинг объявления переменной
        extract-used-registers ;; Извлечение используемых регистров
        parse-numeric-literal ;; Парсинг числового литерала
        parse-while-loop ;; Парсинг while цикла
        parse-function-call ;; Парсинг вызова функции
        parse-switch-case ;; Парсинг switch-case конструкции
        )
...
;; Парсинг присваивания с поддержкой составных операторов и многоуровневых выражений
(defn parse-assignment 
  "Семантический парсинг присваивания с поддержкой составных операторов и многоуровневых выражений

  Расширенная грамматика:
  - Поддержка сложных выражений
  - Многоуровневый AST
  - Полный анализ операций"
  [tokens]
  (log/debug "Начало расширенного парсинга присваивания")
  ...
  (let [[left-token & after-left] tokens]
    (when-not (= (:type left-token) :identifier)
      ...
      (throw (ex-info "Некорректный левый операнд присваивания" 
                      {:tokens tokens})))
    
    (let [[op-token & after-op] after-left]
      (when-not (= (:value op-token) "=")
        (log/error "Ожидается оператор присваивания =")
        (throw (ex-info "Отсутствует оператор присваивания" 
                        {:tokens tokens})))
      
      ;; Парсинг правой части выражения
      (let [right-tokens (drop 1 after-op)
            right-expr (parse-binary-operation right-tokens)]
        
        ;; Создание узла AST для присваивания
        {:type :assignment
...
;; Проверка, является ли последовательность токенов началом объявления функции
...
;; Добавление парсера для циклов for с улучшенной диагностикой
(defn parse-for-loop 
  ...
  (letfn [(parse-primary [tokens]
            (let [[first-token & rest] tokens]
              (cond
                ...
                (= (:value first-token) "(")
                (let [grouped-expr (parse-binary-operation rest)]
          ...
          (parse-with-precedence [tokens precedence]
            (loop [left (parse-primary tokens)]
              (let [[op-token & after-op] (:tokens left)]
                (if (or (nil? op-token)
                        (not (is-binary-operator? op-token))
                        (< (get operator-precedence (keyword (:value op-token)) 0) 
                           precedence))
                  left
                  (let [right (parse-with-precedence 
                               after-op 
                    ...
                    (recur 
                     {:type :binary-operation
                      :left {:type (:expression ast-node-types)
                             :value (:value left)}
                      :operator (:value op-token)
                      :right {:type (:expression ast-node-types)
                              :value (:value right)}
                      :tokens (:tokens right)}))))))]
    
    ;; Начинаем парсинг с самого низкого приоритета
    (parse-with-precedence tokens 0)))
...
;; Семантический парсинг унарных операций с учетом приоритета операторов
...
;; Семантический парсинг битовых операций с учетом приоритета операций
...
;; Семантический парсинг цикла do-while с улучшенной диагностикой
(defn parse-do-while 
...
;; Семантический парсинг операторов break и continue с улучшенной диагностикой
...
;; Семантический парсинг оператора return с улучшенной диагностикой
(defn parse-return 
...
;; Семантический парсинг объявления указателей с улучшенной диагностикой
...
;; Семантический парсинг объявления структур с улучшенной диагностикой
...
;; Семантический парсинг typedef с улучшенной диагностикой
(defn parse-typedef 
...
;; Семантический парсинг с учетом контекста памяти
...
;; Токенизация специальных ключевых слов
...
;; Определение специфичных токенов C51
...
;; Распознавание ключевых слов для специальных регистров (SFR)
